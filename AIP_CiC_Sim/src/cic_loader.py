...
def load_filtered_corpus(csv_path, columns_map_path=None, condition_col="condition", verbose=False):
    ...
    for k,v in mapping.items():
        if v not in df.columns:
            raise ValueError(f"Required column '{v}' not found in CSV. Pass --columns_json if names differ.")
    # Build contexts
    C_list = []
    t_idx = []
    for i,row in df.iterrows():
        ...
        if sc=="target":
            ...
            # skip rows without a clear target
            continue
...
def infer_condition(C, targets):
    import numpy as np
    # crude geometry-based: if both distractors near target => close; both far => far; else split
    ...
    for i in range(N):
        ...
        d12 = abs(dt[0]-dt[1])
        if max(dt) < 0.2:  # both near
            out.append("close")
        elif min(dt) > 0.4:  # both far
            out.append("far")
        else:
            out.append("split")
